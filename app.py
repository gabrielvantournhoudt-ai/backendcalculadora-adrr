            ...
            if is_adr:
                ...
                if regular_price and previous_close:
                    ...
                    if after_hours_price is not None and regular_price not in (None, 0):
                        after_percent = ((after_hours_price - regular_price) / regular_price) * 100
                        after_change_absolute = after_hours_price - regular_price
                        after_market_data = {
                            'price': round_value(after_hours_price, 4),
                            'change': round_value(after_change_absolute, 4),
                            'percent': round_value(after_percent, 2)
                        }

                    pre_market_data = None
                    if pre_market_price is not None and regular_price not in (None, 0):
                        pre_total_percent = ((pre_market_price - regular_price) / regular_price) * 100
                        pre_change_absolute = pre_market_price - regular_price

                        if after_hours_price not in (None, 0):
                            pre_incremental_percent = ((pre_market_price - after_hours_price) / after_hours_price) * 100
                        else:
                            pre_incremental_percent = pre_total_percent

                        pre_market_data = {
                            'price': round_value(pre_market_price, 4),
                            'change': round_value(pre_change_absolute, 4),
                            'percent_total': round_value(pre_total_percent, 2),
                            'percent_incremental': round_value(pre_incremental_percent, 2)
                        }
                    ...
                    return {
                        ...
                        'has_after_market': after_market_data is not None,
                        'has_pre_market': pre_market_data is not None,
                        'data_type': 'closing',
                        'message': 'Dados de fechamento com After/Pré capturados automaticamente',
                        'after_market': after_market_data,
...
class APIHandler(BaseHTTPRequestHandler):
    def do_GET(self):
        ...
        allowed_origins = [
            ...
            'https://calculadorasaldoadr.vercel.app',  # Novo domínio Vercel
    ...
    def do_OPTIONS(self):
        ...
        allowed_origins = [
            ...
            'https://calculadorasaldoadr.vercel.app',
            'http://localhost:3000',
            'http://localhost:8080',
            'http://127.0.0.1:3000',
            '*'
